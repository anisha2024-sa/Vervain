...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .api-status.connected { color: var(--accent3); border-color: var(--accent3); }
  .api-status.disconnected { color: var(--critical); border-color: var(--critical); }
  .api-status.loading { color: var(--warn); border-color: var(--warn); }
...
<header>
  ...
  <div class="header-status">
    ...
    <div class="status-indicator">
      ...
      <span id="criticalHeaderCount">3 CRITICAL PENDING</span>
    </div>
    <div class="status-indicator">
    ...
    <!-- Backend API Status -->
    <div class="api-status loading" id="apiStatus">⬤ API CONNECTING...</div>
  ...
  <div class="tab-panel active" id="tab-overview">
    ...
    <div class="scan-container">
      <div class="scan-info">
        ...
        <div class="scan-target">TARGET: <span style="color:var(--accent)">myapp/node_modules</span> — <span id="packageCount">847</span> packages</div>
      </div>
    ...
    <div class="stat-grid">
      ...
      <div class="stat-card resolved">
        ...
        <div class="stat-value" id="statPatched">--</div>
          ...
          <div class="donut-wrap">
            <div class="donut-chart">
              <svg viewBox="0 0 100 100">
                ...
                <circle id="donutArc" cx="50" cy="50" r="40" fill="none" stroke="var(--accent3)" stroke-width="12"
                  stroke-dasharray="188 251" stroke-linecap="round"/>
              </svg>
              <div class="donut-center">
                <div class="donut-pct" id="scoreValue">75</div>
                <div class="donut-lbl">SCORE</div>
              </div>
            </div>
            <div class="donut-legend">
              <div class="legend-item"><div class="legend-dot" style="background:var(--critical)"></div>Critical: <span id="legendCritical">7</span></div>
              <div class="legend-item"><div class="legend-dot" style="background:var(--high)"></div>High: <span id="legendHigh">23</span></div>
              <div class="legend-item"><div class="legend-dot" style="background:var(--medium)"></div>Medium: <span id="legendMedium">61</span></div>
...
<script>
  ...
  async function fetchStats() {
    try {
      ...
      // Update stat cards
      document.getElementById('statCritical').textContent = String(data.critical).padStart(2,'0');
      document.getElementById('statHigh').textContent = data.high;
      document.getElementById('statMedium').textContent = data.medium;
      document.getElementById('statPatched').textContent = data.autoPatched || 142;

      // Update score donut
      if (data.score) {
        document.getElementById('scoreValue').textContent = data.score;
        const arc = Math.round((data.score / 100) * 251);
        document.getElementById('donutArc').setAttribute('stroke-dasharray', `${arc} 251`);
      }

      // Update legend
      document.getElementById('legendCritical').textContent = data.critical;
      document.getElementById('legendHigh').textContent = data.high;
      document.getElementById('legendMedium').textContent = data.medium || 61;

      // Update header
      document.getElementById('criticalHeaderCount').textContent = `${data.critical} CRITICAL PENDING`;

      // Mark API connected
      setApiStatus('connected');
      addLog('auditLog','INFO',`Backend API connected — stats loaded (critical:${data.critical}, high:${data.high}, score:${data.score})`);

    } catch (err) {
      setApiStatus('disconnected');
      // Fallback to defaults
      document.getElementById('statCritical').textContent = '07';
      document.getElementById('statHigh').textContent = '23';
      document.getElementById('statMedium').textContent = '89';
      document.getElementById('statPatched').textContent = '142';
      addLog('auditLog','WARN','Backend API offline — showing cached data. Start backend with: node src/index.js');
    }
  }
  ...
  async function fetchVulnerabilities() {
    try {
      ...
      const data = await res.json();
      // Merge API data with local vuln data
      if (data && data.length > 0) {
        data.forEach(apiVuln => {
          const match = vulns.find(v => v.pkg === apiVuln.package);
          if (match) {
            match.status = apiVuln.status ? apiVuln.status.toLowerCase() : match.status;
          }
        });
        buildVulnTable();
      }
    } catch (err) {
      // Use local fallback data — already built
    }
  }
  ...
  function setApiStatus(state) {
    ...
    el.textContent = state === 'connected' ? '⬤ API LIVE' :
                     state === 'disconnected' ? '⬤ API OFFLINE' : '⬤ API CONNECTING...';
  }

  // Poll backend every 10 seconds for live updates
  fetchStats();
  ...
  // ============================================================
  // VULN DATA (local fallback + backend overlay)
  // ============================================================
  const vulns = [
